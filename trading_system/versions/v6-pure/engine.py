...
class BacktestEngine:
    ...
    def _step(self, trade_date: date, benchmark_close: float):
        ...
        # 5. Execute sells (in memory) — skip if suspended or limit-down
        sell_blocked = self.constraint_filter.check_sell_constraints(
            trade_date, [code for code, _, _ in exits]
        )
        for code, reason, sell_price in exits:
            if code in sell_blocked:
                continue  # Cannot sell: suspended or limit-down, defer to next trading day
    ...
    def _generate_signals(self, trade_date: date, market_state: MarketState) -> list[Signal]:
        ...
        # Filter out net-negative signals (lock_up_expire, earnings_miss only)
