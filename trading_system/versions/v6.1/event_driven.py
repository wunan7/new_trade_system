...
class EventDrivenStrategy:
    ...
    def _passes_cost_filter(self, event_type: str, confidence: float, factors_dict: dict) -> bool:
        """Heuristic cost-aware filter for event signals.

        We approximate expected move from event type + confidence and require
        either >= 2% expected return or >= 3x one-side trading cost (~0.45%).
        """
        expected_return = 0.0

        if event_type == "龙虎榜":
            expected_return = 0.03 * confidence / 0.55
        elif event_type == "earnings_beat":
            beat_pct = float(factors_dict.get("beat_pct", 0.0))
            expected_return = min(max(beat_pct / 100, 0.01), 0.08)
        elif event_type == "buyback":
            amount = float(factors_dict.get("buyback_amount", 0.0))
            if amount >= 10_000:
        ...
        min_expected = max(0.015, 0.0015 * 3)
        return expected_return >= min_expected
    ...
    def _handle_lhb(self, trade_date: date) -> list[Signal]:
        ...
        for code, count in appearance_count.items():
            ...
            if not self._passes_cost_filter("龙虎榜", confidence, factors):
                continue
            signals.append(self._make_signal(
                trade_date, code, "龙虎榜", confidence, factors
            ))
        return signals
    ...
    def _handle_earnings_beat(self, trade_date: date) -> list[Signal]:
        ...
        for code, event_date, title, content, sentiment, impact in rows:
            ...
            factors = {"beat_pct": beat_pct, "impact": impact or "medium", "reason": title}
            if not self._passes_cost_filter("earnings_beat", confidence, factors):
                continue

            signals.append(self._make_signal(
                trade_date, code, "earnings_beat", confidence, factors
            ))
    ...
    def _handle_buyback(self, trade_date: date) -> list[Signal]:
        ...
        for code, event_date, title, content, sentiment, impact in rows:
            ...
            factors = {"buyback_amount": amount, "impact": impact or "low", "reason": title}
            if not self._passes_cost_filter("buyback", confidence, factors):
                continue

            signals.append(self._make_signal(
                trade_date, code, "buyback", confidence, factors
            ))
        signals.sort(key=lambda s: s.confidence, reverse=True)
