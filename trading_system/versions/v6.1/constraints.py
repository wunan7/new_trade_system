...
COOLDOWN_TRADING_DAYS = 2      # wait 5 trading days after SELL before rebuying
...
class ConstraintFilter:
    ...
    def filter(self, signals, trade_date: date,
        ...
        recent_sell_info = self._load_recent_sell_info(trade_date, codes)
        ...
        for signal in signals:
            ...
            reason = self._check_signal(
                ...
                recent_sell_info=recent_sell_info,
            )
            if reason:
    ...
    def _check_signal(self, code: str, trade_date: date,
                      ...
                      liquidity: dict, portfolio,
                      recent_sell_info: dict[str, int] | None = None) -> str | None:
        ...
        days_since_sell = (recent_sell_info or {}).get(code)
        if days_since_sell is not None and days_since_sell < COOLDOWN_TRADING_DAYS:
            return f"cooldown({days_since_sell}d)"
    ...
    def _load_trade_constraints(self, trade_date: date, codes: list[str]) -> dict:
        ...
        if not codes:
            return {}
        sql = """
            SELECT tc.code, tc.is_suspended, tc.is_st, tc.up_limit, sd.close, tc.down_limit
        ...
        with self.engine.connect() as conn:
            ...
            for row in result:
                ...
                down_limit = float(row[5]) if row[5] else None
                is_limit_up = (close and up_limit and close >= up_limit)
                is_limit_down = (close and down_limit and close <= down_limit)
                out[row[0]] = {
                    "is_suspended": row[1],
                    "is_st": row[2],
    ...
    def _load_recent_sell_info(self, trade_date: date, codes: list[str]) -> dict[str, int]:
        """Return days since most recent SELL for each code, measured in trading days."""
        if not codes:
            return {}

        sql = """
            WITH last_sell AS (
                SELECT code, MAX(trade_date) AS last_sell_date
                FROM trade_log
            ...
            trading_days AS (
                SELECT ls.code, COUNT(DISTINCT sd.trade_date) AS trading_days_since
                FROM last_sell ls
                JOIN stock_daily sd
                  ON sd.trade_date > ls.last_sell_date
                 AND sd.trade_date < :td
                GROUP BY ls.code
            )
            SELECT ls.code, COALESCE(td.trading_days_since, 0)
            FROM last_sell ls
            LEFT JOIN trading_days td ON ls.code = td.code
        """
        with self.engine.connect() as conn:
            result = conn.execute(text(sql), {"td": trade_date, "codes": tuple(codes)})
            return {row[0]: int(row[1]) for row in result}

    def check_sell_constraints(self, trade_date: date, codes: list[str]) -> dict[str, str]:
        """Check if positions can be sold on trade_date.

        Returns dict of {code: reason} for stocks that CANNOT be sold.
        Reasons: 'suspended' (cannot trade), 'limit_down' (cannot sell at limit-down).
        """
        if not codes:
            return {}
        constraints = self._load_trade_constraints(trade_date, codes)
        ...
        for code in codes:
            ...
            if not tc:
                continue
            if tc.get("is_suspended"):
                blocked[code] = "suspended"
            elif tc.get("is_limit_down"):
                blocked[code] = "limit_down"
        return blocked
