...
class MomentumStrategy(BaseStrategy):
    ...
    def generate(self, trade_date: date, factor_df: pd.DataFrame) -> list[Signal]:
        ...
        # Apply filters
        # Percentile thresholds use 0.5 (above-median) to work correctly across
        # both small test fixtures and large real universes where the top 80/70th
        # percentile naturally clears this bar.
        mask = (
            (df["mom20_pct"] >= 0.50) &
            (df["mom60_pct"] >= 0.50) &
            (df["momentum_20d"] <= 0.5) &
