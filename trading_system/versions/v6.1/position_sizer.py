...
MIN_ORDER_AMOUNT = 6_000
LOW_PRICE_THRESHOLD = 8.0
LOW_PRICE_MIN_AMOUNT = 10_000
...
class PositionSizer:
    ...
    def size(self, signals: list[Signal], market_state: MarketState,
        ...
        for sig, target_pct in zip(signals, raw_weights):
            ...
            estimated_amount = shares * sig.entry_price
            if estimated_amount < MIN_ORDER_AMOUNT:
                logger.info(
                    f"Skipping {sig.stock_code} due to minimum order amount: "
                    f"estimated_amount={estimated_amount:.2f} < {MIN_ORDER_AMOUNT}"
                )
                continue
            if sig.entry_price < LOW_PRICE_THRESHOLD and estimated_amount < LOW_PRICE_MIN_AMOUNT:
                logger.info(
                    f"Skipping {sig.stock_code} due to low-price small-order ban: "
                    f"price={sig.entry_price:.2f}, estimated_amount={estimated_amount:.2f} < {LOW_PRICE_MIN_AMOUNT}"
                )
                continue

            actual_pct = estimated_amount / total_value if total_value > 0 else 0
